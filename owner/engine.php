    ...
    try {
        ...
        if (mysqli_num_rows($result1) > 0) {
            while ($rowss = mysqli_fetch_assoc($result1)) {
                ...
                $sql = "UPDATE add_property sets contact_no='$contact_no', property_type='$property_id', estimated_price='$estimated_price', total_rooms='$total_rooms', bedroom='$bedroom', living_room = '$living_room', kitchen='$kitchen', description='description' WHERE owner_id IN (SELECT $owner_id from add_property)";
                $query = mysqli_query($db, $sql);
                if ($query) {
                    $property_id = mysqli_insert_id($db);
                    $countfiles = count($_FILES['p_photo']['name']);
